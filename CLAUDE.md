...
**Local provider `local-sdcpp`** (shape `local`, `src/connectors/local-sdcpp.ts`): runs the machine's own stable-diffusion.cpp `sd-cli` with MiniMax-H3 GGUF (models `minimax-h3-q2/q3/q4`, gated by unified memory 24/32/64 GB). No network, no key, no cost — preflight says "不花钱" and excludes it from the per-second total. It is a **draft tier** (M2 Max 32 GB: 640×384, 39 frames, 4 steps ≈ 216 s, 2-bit quality). The engine never auto-downloads the ~27 GB of weights: missing files → error with exact `curl -C -` commands + license reminder; `ao doctor` reports readiness. Dispatch happens in `generateVideo` before `resolveVideoAccess` (so the "missing API key" check never fires). Frame count snaps to the H3 `17k+5` grid (nearest), width/height to multiples of 32, `--cfg-scale 1.0` is mandatory.
